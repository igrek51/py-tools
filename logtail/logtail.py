...
SHORT_DATE_REGEX = sedRegexReplace("^[0-9]{4}-[0-9]{2}-[0-9]{2} ([0-9]+):([0-9]+):([0-9]+)\\.[0-9]{3} ", Colouring.C_DATE + "\\1:\\2:\\3" + Colouring.C_RESET + " ")
NO_DATE_REGEX = sedRegexReplace("^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]+:[0-9]+:[0-9]+\\.[0-9]+ ", "")
...
class BackgroundExecuteThread(threading.Thread):
	...
	def run(self):
		if self.__cmd2 is None:
			...
			self.__proc = subprocess.Popen(self.__cmd, stdout=None, shell=True)
		else:
			# 2 commands - streams redirection between processes
			self.__proc = subprocess.Popen(self.__cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
			self.__proc2 = subprocess.Popen(self.__cmd2, stdin=self.__proc.stdout, stdout=None, shell=True)
	...
	def stop(self):
		if self.__proc is not None:
			...
			self.__proc = None
		if self.__proc2 is not None:
			if self.__proc2.poll() is None:
				self.__proc2.terminate()
			self.__proc2 = None
...
class Main:
	...
	def tomcatKill(self, listOnly=False):
		psList = self.shellOutput("ps axww")
		tomcatPIDs = []
		for line in string.split(psList, '\n'):
			if "apache-tomcat" in line:
				print("Tomcat process running: " + line)
				values = line.split(" ")
				pid = values[0]
				if not pid:
					pid = values[1]
				tomcatPIDs.append(pid)
		if len(tomcatPIDs) == 0:
			print("no Tomcat process found.")
			return
		if listOnly:
			return
		if len(tomcatPIDs) > 1:
			print("[warning] Tomcat processes count: %d" % len(tomcatPIDs))
		for pid in tomcatPIDs:
			print("killing PID %s..." % pid)
			self.shellExec("kill -9 %s" % pid)
		print("done")
	...
	def printHelp(self):
		...
		print('\t' + 'notrace, tinytrace, shorttrace, fulltrace - format trace')
		...
		print('\t' + 'tomcats - list tomcat processes')
		print('\t' + 'kill - kill all tomcat processes')
	...
	def executeCommand(self, inputCmd):
		...
		elif inputCmd == "tinytrace": # short class name only
			self.TRACE_FILTER = TINY_TRACE_REGEX
		...
		# kill tomcat
			...
			self.tomcatKill()
			return False
		elif inputCmd == "tomcats":
			self.tomcatKill(listOnly=True)
			return False
